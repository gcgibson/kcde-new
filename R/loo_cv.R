...
possible_sigmas <- c(1e-20,1e-19,1e-18,1e-17,1e-16,1e-15,1e-14,1e-13,1e-12,1e-11,1e-10)
possible_etas  <- c(1e-20,1e-10,1e0,1e10,1e20)
...
leave_one_season_out_cv <- function(ts_obj,k,h,num_lags){
    ...
    for (eta in possible_etas){
        ...
        for (season in seasons){
          ...
          for (obs_idx in 1:(length(test_data_from_this_season)-h)){
            pred_i <- predict_seasonal_kcde(c(test_data_from_other_season,test_data_from_this_season[1:obs_idx]),k,h,num_lags,sigma=sigma,eta = eta)
