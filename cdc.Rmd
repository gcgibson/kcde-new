...
for (location in 1:length(region_str)){
    ...
    rbf_fit_ret_list <- fit_rbf(train_data,num_lags = 1,h = 1)
    ...
    for (test_idx  in 1:(length(test_epiweeks)-h)){
      ...
      if (h==1){
        ...
        kcde_pred_dist <-stats::predict(rbf_fit,newdata=test_idx) + 
          stats::predict(rbf_fit,newdata=test_idx+1) +
        stats::predict(rbf_fit2,newdata=test_idx )/2 +
          stats::predict(rbf_fit2,newdata=test_idx +1 )/2+
          stats::predict(rbf_fit3,newdata=test_idx )/6 +
            stats::predict(rbf_fit3,newdata=test_idx +1)/6 +
           logit(test_data[test_idx]/100)
      }
      kcde_pred_dist <- inv.logit(rnorm(10000,kcde_pred_dist,2*rbf_var))*100
